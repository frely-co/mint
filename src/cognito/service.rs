...
use super::{handlers::{handle_admin_initiate_auth, handle_signup}, models::{AdminInitiateAuthRequest, SignUpRequest}};
        ...
        "AWSCognitoIdentityProviderService.SignUp" => {
            let payload: SignUpRequest = match from_slice(body_bytes.as_ref()) {
                ...
                Err(e) => {
                    ...
                    return (
                        StatusCode::BAD_REQUEST,
                        "Invalid JSON for SignUpRequest",
                    )
                        .into_response();
                }
            };
            ...
            Json(sign_up_response).into_response()
        }
